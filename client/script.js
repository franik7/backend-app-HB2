...
function changeColor() {
  sayHelloButton.classList.add("changeColor")
}
...
function changeColorBack() {
  sayHelloButton.classList.remove("changeColor")
  sayHelloButton.classList.add("changeColorBack")
}
...
const ohMy = () => {
  ...
  .then(res => {
    ...
    for(let i = 0; i < res.data.length; i++){
      ...
      let animalsList = document.createElement('p')
      animalsList.textContent = res.data[i];
      document.querySelector('body').appendChild(animalsList);
    }
  }
...
// CODE HERE
function sendAQuery () {
  axios.get('http://localhost:3000/query-test?Apple=pear')
  .then(res => {
    console.log(res.data)
  })
}

document.querySelector('#query-button').addEventListener('click', sendAQuery)
